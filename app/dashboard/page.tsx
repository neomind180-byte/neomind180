...
function ProgressSummary() {
  ...
  return (
    <div className="bg-[var(--bg-card)] p-8 rounded-[2.5rem] border border-[var(--border)] shadow-sm space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h3 className="text-lg font-black text-[var(--text-primary)] uppercase tracking-tight">Momentum</h3>
          <p className="text-[9px] font-black uppercase tracking-widest text-[var(--text-muted)]">7-Day Streak</p>
        </div>
      ...
      <div className="flex justify-between items-center">
        {days.map((day, i) => (
          <div key={i} className="flex flex-col items-center gap-2">
            <span className="text-[8px] font-black text-[var(--text-dim)] uppercase">{day}</span>
            <div className={`w-7 h-7 rounded-full flex items-center justify-center transition-all ${weeklyStreak[i] ? "bg-[#0AA390] text-white shadow-lg shadow-[#0AA390]/20" : "bg-[var(--bg-input)] text-[var(--text-dim)]"
              }`}>
              {weeklyStreak[i] ? <CheckCircle2 className="w-3.5 h-3.5" /> : <div className="w-1 h-1 rounded-full bg-[var(--border)]" />}
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-3 pt-4 border-t border-[var(--border)]">
        <h4 className="text-[8px] font-black uppercase tracking-[0.2em] text-[var(--text-muted)]">Last Observations</h4>
        {recentShifts.length > 0 ? recentShifts.map((shift, idx) => (
          <div key={idx} className="flex justify-between items-center p-3 bg-[var(--bg-input)] rounded-xl border border-[var(--border)]">
            ...
            <div className="flex gap-2">
              <span className="text-[8px] font-black uppercase px-2 py-1 bg-[var(--bg-card)] rounded-md text-[#00538e] border border-[var(--border)]">{shift.mind}</span>
              <span className="text-[8px] font-black uppercase px-2 py-1 bg-[var(--bg-card)] rounded-md text-[#0AA390] border border-[var(--border)]">{shift.body}</span>
            </div>
          </div>
        )) : (
          <p className="text-[9px] text-[var(--text-dim)] italic text-center py-2">No recent check-ins found.</p>
        )}
      </div>
    </div>
  );
    ...
    <div className="space-y-10 pb-20">
      ...
      <header className="flex justify-between items-center">
        <div>
          ...
          <p className="text-[10px] font-black text-[var(--text-secondary)] uppercase tracking-[0.3em] mt-3">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link
            href="/"
            className="flex items-center gap-2 px-6 py-3 bg-[var(--bg-card)] border border-[var(--border)] rounded-full text-[10px] font-black uppercase text-[var(--text-muted)] hover:text-[#993366] hover:bg-red-500/10 transition-all"
          >
      ...
      <section className="grid md:grid-cols-2 gap-6">
        <div className="p-8 rounded-[2.5rem] bg-[var(--bg-card)] border border-[var(--border)] shadow-sm relative overflow-hidden group hover:border-[#00538e]/30 transition-all">
          ...
          <p className="text-lg font-medium text-[var(--text-secondary)] leading-relaxed italic">
            &quot;{scriptureQuotes[dayIndex].text}&quot;
          </p>
          <p className="text-[10px] text-[var(--text-muted)] font-bold uppercase mt-6">
            {scriptureQuotes[dayIndex].ref}
          </p>
        </div>

        <div className="p-8 rounded-[2.5rem] bg-[var(--bg-card)] border border-[var(--border)] shadow-sm relative overflow-hidden group hover:border-[#0AA390]/30 transition-all">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <div className="bg-[var(--bg-card)] p-10 rounded-[3rem] border border-[var(--border)] shadow-sm">
            <div className="flex justify-between items-start mb-8">
              <div>
                <h3 className="text-xl font-black text-[var(--text-primary)] uppercase tracking-tight">Daily Check-In</h3>
                <p className="text-sm text-[var(--text-secondary)] mt-2 italic font-medium">Acknowledge your internal climate.</p>
              </div>
            ...
            <div className="space-y-4 py-8 border-y border-[var(--border)]">
              {['Mind', 'Body', 'Energy'].map((label, i) => (
                <div key={i} className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                  <span className="text-[var(--text-muted)]">{label}:</span>
                  <div className="flex gap-4 text-[var(--text-dim)]">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <h3 className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] px-4">Direct Access</h3>
              ...
              {[
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[11px] font-bold text-[var(--text-secondary)] uppercase tracking-tight">{item.label}</span>
                  </div>
