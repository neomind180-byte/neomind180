...
export default function HistoryPage() {
  ...
  if (loading) return <div className="p-10 text-center text-[var(--text-dim)] font-black uppercase tracking-widest text-[10px]">Loading History...</div>;
            ...
            <div key={item.id} className="relative flex items-start gap-8 group">
              ...
              <div className="flex-grow bg-[var(--bg-card)] border border-[var(--border)] p-8 rounded-[2.5rem] shadow-sm">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="text-[10px] font-bold text-[var(--text-dim)] uppercase tracking-wide">
                ...
                {item.type === 'check-in' ? (
                  <div className="grid grid-cols-3 gap-2">
                    {['Mind', 'Body', 'Energy'].map(k => (
                      <div key={k} className="bg-[var(--bg-primary)] px-3 py-2 rounded-xl border border-[var(--border)]">
                        <p className="text-[9px] uppercase text-[var(--text-muted)] font-bold">{k}</p>
                        <p className="text-xs font-bold text-[var(--text-secondary)]">{item.data[k.toLowerCase()]}</p>
                      </div>
                    ))}
                  </div>
                    ...
                    {expandedId === item.id && (
                      <div className="mt-4 pt-4 border-t border-[var(--border)] space-y-4 max-h-60 overflow-y-auto pr-2">
                        {item.data.messages.map((msg: any, mIdx: number) => (
                          <div key={mIdx} className={`p-4 rounded-2xl text-[11px] leading-relaxed ${msg.role === 'neo' ? 'bg-[var(--bg-primary)] text-[var(--text-secondary)] border border-[var(--border)]' : 'bg-[#00538e] text-white'
                            }`}>
                  ...
                  <div className="space-y-3">
                    <p className="text-sm text-[var(--text-secondary)] italic">"{item.data.thought}"</p>
                    <div className="pl-4 border-l-2 border-[#993366]/20">
                      ...
                      <p className="text-sm font-medium text-[var(--text-secondary)]">{item.data.new_perspective}</p>
                    </div>
                  </div>
                )}
              </div>
